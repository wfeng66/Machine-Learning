...
def main():
    ...
    slice_len = math.floor(len(df)/n)
    # General NaiveBayes Classification
    for iterTime in range(n):
        trainingdt = df[slice_len*(iterTime+1):]+df[slice_len*(iterTime-1):slice_len*iterTime]
        testingdt = df[slice_len*iterTime:slice_len*(iterTime+1)]
    ...
    print ("-"*50)
    print ("Sentiment\tmin_acc\tavg_acc\tmax_acc\n")
    print ("*"*50)
    print ("noon\t\t%f\t%f\t%f\n" % (acc_gen_min,acc_gen_avg,acc_gen_max))
    ...
    for sent in range(10):
        ...
        print ("%f\t%f\t%f\t%f\n" % (sent/10,acc_sent_min,acc_sent_avg,acc_sent_max))
