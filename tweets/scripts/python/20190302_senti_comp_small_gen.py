...
def main():
    ...
    print ("*"*100)
    for size in range(50, 5050, 500):
        # General NaiveBayes Classification
        for iterTime in range(n):
            if (500+size*(iterTime+1))>6020:
                trainingdt = df[(500+size*iterTime):6020]+df[0:(size-(6020-(500+size*iterTime)))]
            else: 
                trainingdt = df[(500+size*iterTime):(500+size*(iterTime+1))]
            testingdt = df[(500*iterTime)%6000:(500*(iterTime+1))%6000]
